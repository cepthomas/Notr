...
try:
    ...
    sublime.message_dialog('Notr plugin requires SbotCommon plugin')
    raise ImportError('Notr plugin requires SbotCommon plugin')
...
#-----------------------------------------------------------------------------------
class NotrMarkdownCommand(sublime_plugin.TextCommand):
    ''' Turn md into html.'''
...
def _output_html(view, content=None):
    ...
    #     view.set_syntax_file('Packages/HTML/HTML.tmLanguage')
        ...
        temp_path = os.path.join(sublime.packages_path(), 'Notr', 'temp')
